...
export function initCards() {
  ...
  // 金币/碎片变化时重新渲染网格（红点可升级状态可能改变）
  gameState.on('coins-changed', () => {
    if (document.getElementById('screen-cards')?.classList.contains('active')) renderGrid();
  });
...
function renderGrid() {
  ...
  grid.innerHTML = filtered.map(char => {
    ...
    const canUpgradeCard = gameState.canUpgradeCard(char.id);
    ...
    return `
      <div style="border-radius:12px;cursor:pointer;position:relative;
        ...
        <!-- 稀有度标签（放大） -->
        ${rLabel ? `<div style="position:absolute;top:4px;left:4px;font-size:11px;color:${rb};font-weight:800;line-height:1">${rLabel}</div>` : ''}
        <!-- 可升级红点 -->
        ${canUpgradeCard ? `<div style="position:absolute;top:4px;right:4px;width:10px;height:10px;border-radius:50%;background:#f44336;box-shadow:0 0 4px rgba(244,67,54,0.7)"></div>` : ''}
        <!-- Q版大头像 -->
        <div style="margin:-8px auto 2px">${avatarHTML(char.id, 110)}</div>
        <!-- 名字 + 等级 -->
        <div style="font-size:14px;font-weight:800;display:flex;align-items:center;justify-content:center;gap:4px">
          ${char.name}
          <span style="font-size:10px;font-weight:700;color:var(--text-light);background:rgba(0,0,0,0.07);border-radius:4px;padding:1px 4px">Lv${level}</span>
        </div>
        <!-- 星级 -->
...
function getKingdomName(kingdom) {
  return { wei: '魏', shu: '蜀', wu: '吴', qun: '群雄' }[kingdom] || '';
}
